...
def parse_cli() -> argparse.Namespace:
    ...
    parser.add_argument('--profile', '-p', type=str, default='',
        choices={'all_examples', 'access', 'basic', 'full_nfv', 'on_prem', 'regional_dc', 'remote_fp', 'storage', 'ifpd','build_your_own'}, # add new profiles here
        help='''profile name which files, required in deployment, will be copied to the project root directory''')
    parser.add_argument('--arch', '-a', type=str, default='icx', choices={"icx", "clx", "skl", "spr",'adl'})  # please add acronyms for new architectures here
